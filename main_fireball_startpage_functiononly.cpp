...
#include "start.cpp"
#include "volcano_functiononly.cpp"
...
int main( int argc, char* args[] )
{
  ...
  // NPC_FIREBALL fireball2(BLOCK_WIDTH,BLOCK_WIDTH*2);//fireball's start position
    ...
    int vol1_x=rand()%14+1,vol1_y=rand()%8+1,vol2_x=rand()%14+1,vol2_y=rand()%8+1,vol3_x=rand()%14+1,vol3_y=rand()%8+1,random_erupt=rand()%3;
    bool hurt=false,change_dir=false,next=false;
    bool vol1alone=false,vol1erupt=false,vol2alone=false,vol2erupt=false,vol3alone=false,vol3erupt=false;
    typedef unsigned long clock_t;
    NPC_FIREBALL fireball(BLOCK_WIDTH,BLOCK_HEIGHT*fireball_start_y);//fireball's start position
    //volcano1(rand()%14+1,rand()%8+1);volcano2(rand()%14+1,rand()%8+1);volcano3(rand()%14+1,rand()%8+1);//generate 3 volcanoes
...
volcano1(&hp_width,&minion,fireball.get_fireball_position_x(),fireball_start_y*BLOCK_HEIGHT,&banana_position,vol1alone,vol1erupt,vol1_x,vol1_y,vol2_x,vol2_y,vol3_x,vol3_y,random_erupt);
volcano2(&hp_width,&minion,fireball.get_fireball_position_x(),fireball_start_y*BLOCK_HEIGHT,&banana_position,vol2alone,vol2erupt,vol1_x,vol1_y,vol2_x,vol2_y,vol3_x,vol3_y,random_erupt);
volcano3(&hp_width,&minion,fireball.get_fireball_position_x(),fireball_start_y*BLOCK_HEIGHT,&banana_position,vol3alone,vol3erupt,vol1_x,vol1_y,vol2_x,vol2_y,vol3_x,vol3_y,random_erupt);
...
hurt_of_volcanoes(vol1alone,vol1erupt,vol2alone,vol2erupt,vol3alone,vol3erupt);
...
if(hurt){vol1_x=rand()%14+1;vol1_y=rand()%8+1;vol2_x=rand()%14+1;vol2_y=rand()%8+1;vol3_x=rand()%14+1;vol3_y=rand()%8+1;}
